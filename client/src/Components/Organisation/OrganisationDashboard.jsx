...
const OrganisationDashboard = () => {
   ...
   const [error , setError] = useState(false)
    const {organisation , isOrganisation,setIsOrganisation}=useAuth()
    console.log('in Organisation dahboard')
    ...
    const handleDelete = async ()=>{
          await axios.delete(`${backendURL}api/v1/organisation/${organisation._id}/delete-account`,{withCredentials : true})
          .then((res)=>{
            console.log('deleted')
            setIsOrganisation(false)
             navigate('/')
